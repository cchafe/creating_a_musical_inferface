...
#include "duplex.h"
#include "globals.h"
// PIPEWIRE_LATENCY=32/48000 ./hacktrip
...
Duplex::Duplex(QObject *parent) : QThread(parent) {
    // default values
    unsigned int bufferFrames = 64; // depends on what the operating system can do
...
//                         &gAudio_callback, (void *)&m_bufferBytes, &options);
    m_adac->openStream(&m_oParams, &m_iParams, FORMAT, m_fs, &bufferFrames,
                       &Duplex::wrapperProcessCallback, (void *)this, &options);
...
void Duplex::setNetworkRcv(UDP *rcv) {
    mUdpRcv  = rcv;
};
...
//*******************************************************************************
int Duplex::wrapperProcessCallback(void *outputBuffer, void *inputBuffer,
                                   unsigned int nBufferFrames, double streamTime,
                                   RtAudioStreamStatus status, void *arg)
{
    std::cout << "Stream xxxxxxxxxxxx" << std::endl;
    return static_cast<Duplex*>(arg)->networkAudio_callback(
                outputBuffer, inputBuffer, nBufferFrames, streamTime, status, arg);
}

int Duplex::networkAudio_callback
